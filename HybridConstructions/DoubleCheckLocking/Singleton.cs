...
namespace DoubleCheckLocking
{
    ...
    {
        // закрытый конструктор, чтобы никто не мог создать экземпляр последством new
        private Singleton() { }
        ...
        public static Singleton GetSingleton()
        {
            ...
            #region Пояснение
            // так как компилятор может сначала выделить память для объекта, а поток вызвать его конструктор,
